...
function Ground() {
    ...
    const checkScrollable = () => {
        if (contentBodyRef.current) {
            setIsScrollable(contentBodyRef.current.scrollHeight > contentBodyRef.current.clientHeight);
        }
    };
                    ...
                    <div className='popup' onClick={(e) => e.stopPropagation()}>
                        <div className='tree-popup-content'>
                            ...
                            <div className='wish-container'>
                                ...
                                <div 
                                    className={`popup-body ${isScrollable ? 'scrollable' : 'centered'}`} 
                                    ref={contentBodyRef}
                                >
                                    <div className='popup-content-body'>
                                        {wishes.length === 0 ? "아무 사용자가 소원을 달지 않았습니다" : wishes[currentWishIndex]?.contents}
                                    </div>
                                </div>
                            </div>
